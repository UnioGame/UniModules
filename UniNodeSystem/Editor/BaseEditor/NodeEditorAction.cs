...
namespace UniNodeSystemEditor {
    public partial class NodeEditorWindow {
        ...
        private UniNodeSystem.UniBaseNode hoveredNode = null;
        [NonSerialized] private UniNodeSystem.NodePort hoveredPort = null;
        [NonSerialized] private UniNodeSystem.NodePort draggedOutput = null;
        [NonSerialized] private UniNodeSystem.NodePort draggedOutputTarget = null;
        ...
        private struct RerouteReference {
            public UniNodeSystem.NodePort port;
            public int connectionIndex;
            public int pointIndex;

            public RerouteReference(UniNodeSystem.NodePort port, int connectionIndex, int pointIndex) {
                this.port = port;
                    ...
                    if (e.button == 0) {
                        ...
                        if (currentActivity == NodeActivity.DragNode) {
                            ...
                            for (int i = 0; i < Selection.objects.Length; i++) {
                                if (Selection.objects[i] is UniNodeSystem.UniBaseNode) {
                                    UniNodeSystem.UniBaseNode node = Selection.objects[i] as UniNodeSystem.UniBaseNode;
                                    ...
                                    if (offset.sqrMagnitude > 0) {
                                        foreach (UniNodeSystem.NodePort output in node.Outputs) {
                                            Rect rect;
                                        ...
                                        foreach (UniNodeSystem.NodePort input in node.Inputs) {
                                            Rect rect;
                        ...
                        if (IsHoveringPort) {
                            if (hoveredPort.IsOutput) {
                                ...
                                if (hoveredPort.IsConnected) {
                                    UniNodeSystem.UniBaseNode node = hoveredPort.node;
                                    UniNodeSystem.NodePort output = hoveredPort.Connection;
                    ...
                    if (e.button == 0) {
                        ...
                        if (IsDraggingPort) {
                            ...
                            if (draggedOutputTarget != null) {
                                UniNodeSystem.UniBaseNode node = draggedOutputTarget.node;
                            ...
                            IEnumerable<UniNodeSystem.UniBaseNode> nodes = Selection.objects.Where(x => x is UniNodeSystem.UniBaseNode).Select(x => x as UniNodeSystem.UniBaseNode);
                            foreach (UniNodeSystem.UniBaseNode node in nodes) EditorUtility.SetDirty(node);
                            if (NodeEditorPreferences.GetSettings().autoSave) AssetDatabase.SaveAssets();
                        } else if (!IsHoveringNode) {
        ...
        public void RemoveSelectedNodes() {
            ...
            foreach (UnityEngine.Object item in Selection.objects) {
                if (item is UniNodeSystem.UniBaseNode) {
                    UniNodeSystem.UniBaseNode node = item as UniNodeSystem.UniBaseNode;
        ...
        public void RenameSelectedNode() {
            if (Selection.objects.Length == 1 && Selection.activeObject is UniNodeSystem.UniBaseNode) {
                UniNodeSystem.UniBaseNode node = Selection.activeObject as UniNodeSystem.UniBaseNode;
                NodeEditor.GetEditor(node).InitiateRename();
            }
        }

        /// <summary> Draw this node on top of other nodes by placing it last in the graph.nodes list </summary>
        public void MoveNodeToTop(UniNodeSystem.UniBaseNode node) {
            int index;
        ...
        public void DublicateSelectedNodes() {
            ...
            Dictionary<UniNodeSystem.UniBaseNode, UniNodeSystem.UniBaseNode> substitutes = new Dictionary<UniNodeSystem.UniBaseNode, UniNodeSystem.UniBaseNode>();
            for (int i = 0; i < Selection.objects.Length; i++) {
                if (Selection.objects[i] is UniNodeSystem.UniBaseNode) {
                    UniNodeSystem.UniBaseNode srcNode = Selection.objects[i] as UniNodeSystem.UniBaseNode;
                    if (srcNode.graph != graph) continue; // ignore nodes selected in another graph
                    UniNodeSystem.UniBaseNode newNode = graphEditor.CopyNode(srcNode);
            ...
            for (int i = 0; i < Selection.objects.Length; i++) {
                if (Selection.objects[i] is UniNodeSystem.UniBaseNode) {
                    UniNodeSystem.UniBaseNode srcNode = Selection.objects[i] as UniNodeSystem.UniBaseNode;
                    if (srcNode.graph != graph) continue; // ignore nodes selected in another graph
                    foreach (UniNodeSystem.NodePort port in srcNode.Ports) {
                        for (int c = 0; c < port.ConnectionCount; c++) {
                            UniNodeSystem.NodePort inputPort = port.direction == UniNodeSystem.PortIO.Input ? port : port.GetConnection(c);
                            UniNodeSystem.NodePort outputPort = port.direction == UniNodeSystem.PortIO.Output ? port : port.GetConnection(c);

                            UniNodeSystem.UniBaseNode newNodeIn, newNodeOut;
        ...
        bool IsHoveringTitle(UniNodeSystem.UniBaseNode node) {
